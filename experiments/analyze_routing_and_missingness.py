...
"""Revision-7 clean, fairness-controlled missingness, and routing ablations.
...
DEFAULT_OUTPUT = Path("artifacts/revision7/comparative_analysis")
...
def run(args: argparse.Namespace) -> int:
    ...
    if not output.is_relative_to(allowed):
        raise ValueError("Revision-7 analysis output must remain under artifacts/revision7")
    ...
    if len(devices) < 3 or not torch.cuda.is_available():
        raise RuntimeError("Revision-7 missingness analysis requires at least three GPUs")
    ...
    for (mechanism, rate), corruption_seed in (
        ...
        if cached is None and rate == 0.0:
            cached = a6_ablation_cache.get(
                (runner.ROBUSTNESS_MECHANISMS[0], 0.0, corruption_seed)
            )
        if cached is None:
    ...
    if input_before != input_after:
        raise RuntimeError("registered training file changed during Revision-7 analysis")
    payload = {
...
def parser() -> argparse.ArgumentParser:
    ...
    value.add_argument("--source", default="artifacts/revision7/wlcr_sea")
    value.add_argument(
        "--clean-neural-root",
